...
class test_network(base_page):
    def __init__(self, driver=None):
        super().__init__(driver)
        self.meau_expand = self.config.get("network_config", "meau_expand").split(', ')
        self.path = [path.strip() for path in self.config.get("network_config", "path").splitlines() ]
        self.network_config = [config.strip() for config in self.config.get("network_config", "config").splitlines()]

    def network_config_auto(self):
        # 判断 BMC设置 菜单是否展开
        ...
        self.webdriver.save_screenshot(r"backend\screenshot_save\network_config.png")        
