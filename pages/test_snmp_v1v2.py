...
class test_snmp_v1v2(base_page):
    def __init__(self, driver=None):
        super().__init__(driver)
        self.meau_expand = self.config.get("snmp_v1v2_config", "meau_expand").split(', ')
        self.path = [path.strip() for path in self.config.get("snmp_v1v2_config", "path").splitlines() ]
        self.snmp_v1v2_config = [config.strip() for config in self.config.get("snmp_v1v2_config", "config").splitlines()]

    def snmp_v1v2_config_auto(self):
        # 判断 BMC设置 菜单是否展开
        ...
        import time 
        # 自动操作
        for config in self.snmp_v1v2_config:
        ...
        self.webdriver.save_screenshot(r"backend\screenshot_save\snmp_v1v2_config.png")        
