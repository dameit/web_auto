...
class test_ldap(base_page):
    def __init__(self, driver=None):
        super().__init__(driver)
        self.meau_expand = self.config.get("ldap_config", "meau_expand").split(', ')
        self.path = [path.strip() for path in self.config.get("ldap_config", "path").splitlines() ]
        self.ldap_config = [config.strip() for config in self.config.get("ldap_config", "config").splitlines()]

    def ldap_config_auto(self):
        # 判断 BMC设置 菜单是否展开
        ...
        # 自动操作
        for config in self.ldap_config:
            ...
            if config_type == "click":
                ...
                self.click_element(element=element)
                self.webdriver.save_screenshot(rf"backend\screenshot_save\ldap_config_{index}.png")   
                index += 1
