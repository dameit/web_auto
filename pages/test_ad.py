...
class test_ad(base_page):
    def __init__(self, driver=None):
        super().__init__(driver)
        self.meau_expand = self.config.get("ad_config", "meau_expand").split(', ')
        self.path = [path.strip() for path in self.config.get("ad_config", "path").splitlines() ]
        self.ad_config = [config.strip() for config in self.config.get("ad_config", "config").splitlines()]

    def ad_config_auto(self):
        # 判断 BMC设置 菜单是否展开
        ...
        for config in self.ad_config:
            ...
            if config_type == "click":
                ...
                self.click_element(element=element)
                self.webdriver.save_screenshot(rf"backend\screenshot_save\ad_config_{index}.png")   
                index += 1
